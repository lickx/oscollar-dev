...
integer Auth(string sObjectID, string sUserID)
{
    ...
    else if (llListFindList(g_lBlockObj,[sObjectID]) != -1) return -1;
    else if (llListFindList(g_lBlockAv+g_lBlock,[sOwner]) != -1) return -1;
    ...
    if ((key)sUserID) {
        ...
        else if (llListFindList(g_lOwner+g_lTrust+[g_sTempOwner],[sUserID]) != -1) {}
        else return 0;
    }
    return iAuth;
}
...
string HandleCommand(string sIdent, key kID, string sCom, integer iAuthed)
{
    ...
    for (i = 0; i < llGetListLength(lCommands); ++i) {
        ...
        if (sCom == "!release" || sCom == "@clear") {
            ...
            sAck = "";
        } else if (llSubStringIndex(sCom, "@version") == 0 || llSubStringIndex(sCom,"@get") == 0 || llSubStringIndex(sCom,"@findfolder") == 0) {
            if ((integer)sVal) llMessageLinked(LINK_RLV, RLV_CMD, llGetSubString(sCom,1,-1), kID);
            else sAck = "ko";
        } else if (iAuthed == FALSE) return "need auth";
...
Menu(key kID, integer iAuth)
{
    ...
    if (g_iHelpless == FALSE) lButtons+=["SAFEWORD"];
    if (g_sSourceID != "")
...
refreshRlvListener()
{
    ...
    if (g_iRLV && g_iBaseMode && g_iRecentSafeword == FALSE) {
...
UserCommand(integer iAuth, string sStr, key kID)
{
    ...
    if (g_iRLV == FALSE) {
    ...
    else if (llGetSubString(sStr, 6, -1) == "safeword") SafeWord();
        ...
        if (index != -1) sChangetype = llGetSubString(sStr, 0, index-1);
        ...
        if (sChangetype == "helpless") {
            ...
            if (iModeType != -1) {
                if (iModeType >= g_iMinBaseMode) {
                    if (iModeType > 0) sText = "/me is set to "+llList2String(lModes,iModeType)+" mode.";
                    else sText = "/me is offline.";
                    g_iBaseMode = iModeType;
                } else iWSuccess = 1;
            }
        }
        if (iWSuccess == 0) RelayNotify(kID, sText, 1);
...
default
{
            ...
            if (iMenuIndex != -1) {
                ...
                if (sMenu == "Menu~Main") {
                    ...
                    if (sMsg == "Yes") {
                        ...
                        if (g_kObjectUser != NULL_KEY) g_sTempTrustUser = g_kObjectUser;
                        iAuth = 1;
                        ...
                        if (g_kObjectUser != NULL_KEY) {
                            if (llListFindList(g_lBlockAv,[(string)g_kObjectUser]) == -1) {
                            ...
                            if (llListFindList(g_lBlockObj,[sCurID]) == -1) {
                    ...
                    if (iAuth == 1) HandleCommand(sIdent, sCurID, sCom, TRUE);
                        ...
                        for (j = 0; j < llGetListLength(lCommands); ++j) {
                            ...
                            if (llSubStringIndex(sCommand,"@") == 0)
    ...
    timer()
    {
        ...
        while ((iAuthMenuIndex = llListFindList(g_lMenuIDs,["AuthMenu"])) != -1)
            g_lMenuIDs = llDeleteSubList(g_lMenuIDs, iAuthMenuIndex-2, iAuthMenuIndex-3+g_iMenuStride);
    }
}
