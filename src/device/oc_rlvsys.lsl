...
DoMenu(key kID, integer iAuth)
{
    ...
    if (iIndex != -1) g_lMenuIDs = llListReplaceList(g_lMenuIDs, [kID,kMenuID,g_sSubMenu], iIndex, iIndex + g_iMenuStride-1);
...
rebakeSourceRestrictions(key kSource)
{
    ...
    if (iSourceIndex != -1) {
        ...
        while(llGetListLength(lRestr) > 0) {
...
setRlvState()
{
    if (g_iRLVOn && g_iViewerCheck) {
        ...
        g_iRlvActive=FALSE;
        while (llGetListLength(g_lBaked) > 0) {
...
SafeWord(key kID)
{
    ...
    while (numRestrictions) {
        numRestrictions -= 2;
        string sSource = llList2String(g_lRestrictions, numRestrictions);
        if (sSource != "main" && sSource != "rlvex" && llSubStringIndex(sSource, "utility_") != 0)
            llMessageLinked(LINK_THIS, RLV_CMD, "clear", sSource);
    }
...
UserCommand(integer iAuth, string sStr, key kID)
{
    ...
    if (sStr == "runaway" && kID == g_kWearer) {
        ...
        integer numRestrictions = llGetListLength(g_lRestrictions);
        if (numRestrictions == 0) sOut = "There are no restrictions right now.";
        while (numRestrictions > 0) {
            key kSource = llList2Key(g_lRestrictions, numRestrictions-2);
            if (kSource != NULL_KEY)
        ...
        if (iNum == MENUNAME_REQUEST && sStr == g_sParentMenu) {
            ...
            if (iMenuIndex != -1) {
                ...
                if (sMenu == g_sSubMenu) {
                    ...
                    else if (llListFindList(g_lMenu, [sMsg]) != -1)
                ...
                while (llGetListLength(lCommands) > 0) {
                    ...
                    if (sVal == "n" || sVal == "add") AddRestriction(kID, sCom);
                        ...
                        if ((string)kID == "rlvex") {
                            ...
                            while (llGetListLength(lSrcRestr) > 0) {
                            ...
                            lSrcRestr = [];
                            while(llGetListLength(lRestrictionsToRemove) > 0) {
        ...
        if (g_iWaitRelay) {
            if (g_iWaitRelay < 2) {
                ...
                for (i = 0; i < llGetListLength(g_lRestrictions)/2; i++) {
                    key kSource = llList2Key(llList2ListStrided(g_lRestrictions, 0, -1, 2), i);
