...
ListenControl()
{
    ...
    if (g_iIsEnabled && llGetListLength(g_lBadWords) > 0) g_iListenerHandle = llListen(0, "", g_kWearer, "");
}
...
string WordPrompt()
{
    ...
    if (iLength == 0) sPrompt = "%WEARERNAME% is not forbidden from saying anything.";
...
UserCommand(integer iAuth, string sStr, key kID, integer remenu)
{
    ...
    if (llToLower(sStr) == "badwords" || llToLower(sStr) == "menu badwords") {
        ...
        if (sCommand == "add") {
            ...
            if (llGetListLength(lNewBadWords) > 0) {
                while (llGetListLength(lNewBadWords) > 0) {
                    string sNewWord=llToLower(DePunctuate(llList2String(lNewBadWords,-1)));
                    if (remenu) {
                        string sCRLF= llUnescapeURL("%0A");
                        if (llSubStringIndex(sNewWord, sCRLF) != -1) {
                    ...
                    if (llSubStringIndex(g_sPenance, sNewWord) != -1)
                ...
                if (llGetListLength(g_lBadWords) > 0) {
            ...
            if (llGetListLength(lParams) > 2) {
                ...
                if (llListFindList(g_lAnims,[sName]) != -1 || g_iDefaultAnim) {
            ...
            if (llGetListLength(lParams) > 2){
                ...
                for (i = 0; i < iMax; ++i) {
                    ...
                    if (sName != "" && llSubStringIndex(sName,"~") == 0) lSoundList += [sName];
                }
            ...
            if (llGetListLength(lParams) > 2){
                ...
                for (i = 0; i < llGetListLength(g_lBadWords); ++i) {
                    ...
                    if (llSubStringIndex(sPenance,sCheckWord) != -1) {
                         lTemp += [sCheckWord];
                    }
                }
            ...
            if (llGetListLength(lNewBadWords) > 0) {
                while (llGetListLength(lNewBadWords) > 0) {
                    ...
                    if (iIndex != -1) g_lBadWords = llDeleteSubList(g_lBadWords, iIndex, iIndex);
                    lNewBadWords = llDeleteSubList(lNewBadWords, -1, -1);
                }
                llMessageLinked(LINK_SAVE, LM_SETTING_SAVE, g_sSettingToken+"words=" + llDumpList2String(g_lBadWords, ","), "");
                llMessageLinked(LINK_DIALOG, NOTIFY, "0"+WordPrompt(), kID);
                ...
                if (llGetListLength(g_lBadWords) > 0) Dialog(kID, "Select a badword to remove or clear them all.", g_lBadWords, ["Clear", "BACK"],0, iAuth, "BadwordsRemove");
                else {
...
default
{
    ...
    {
        if (llSubStringIndex(llToLower(sMessage),llToLower(g_sPenance)) != -1 && g_iHasSworn) {
            if (g_sBadWordSound != g_sNoSound) llStopSound();
            llMessageLinked(LINK_ANIM, ANIM_STOP, g_sBadWordAnim, "");
            llMessageLinked(LINK_DIALOG, NOTIFY, "0"+"Penance accepted.", g_kWearer);
            g_iHasSworn = FALSE;
        } else if (llSubStringIndex(sMessage, "rembadword") != -1) return;
        else {
            sMessage = llToLower(sMessage);
            list lWords = llParseString2List(sMessage, [" "], []);
            while (llGetListLength(lWords) > 0) {
                ...
                if (llListFindList(g_lBadWords, [sWord]) != -1) {
                    if(g_sBadWordSound != g_sNoSound) {
                        if (g_sBadWordSound == "Default") llLoopSound("~badwordsound", 1.0);
